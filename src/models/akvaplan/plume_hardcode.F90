...
module akvaplan_plume_hardcode
   ...
   type,extends(type_base_model),public :: type_plume_hardcode
...
contains
   ...
   subroutine initialize(self,configunit)
      class (type_plume_hardcode),intent(inout),target :: self
      integer,                     intent(in)           :: configunit

      ! Obtain parameter values
      call self%get_parameter(self%rho, 'rho', 'kg m-3', 'plume density')
      call self%get_parameter(self%plume_layer, 'plume_layer', 'siglay', 'integer sigma layer')
   ...
   subroutine do_column(self,_ARGUMENTS_VERTICAL_)
      class (type_plume_hardcode),intent(in) :: self
      ...
      _UPWARD_LOOP_BEGIN_
         ...
         if (countlayer == self%plume_layer) then
            ! Target layer found - convert the depth-integrated tracer flux into
            ! a local source term by dividing by layer height, then return.
            _GET_(self%id_h,h)
            _SET_DIAGNOSTIC_(self%id_flux,flux_int/h)
            return
         end if

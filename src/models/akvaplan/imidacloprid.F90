...
contains
   ...
   subroutine do(self,_ARGUMENTS_DO_)
      ...
      real(rk) :: c, T, f, DT50R, kphoto, crate, Iz
      ...
      _LOOP_BEGIN_
         ...
         _SET_ODE_(self%id_c,+c*crate) !obs:crate already negative
   ...
   ! !LOCAL VARIABLES:
      real(rk) :: c
      real(rk) :: c_bot
      real(rk) :: shear,erosion,h,kmicrob,cratem
      real(rk), parameter :: DT50M = 3840.0_rk  !Microbial half life in hours according to (Roberts, T. R. and Hutson, D. H., 1999), 720 best case 3840 worst case
   !EOP
   !----------------------------------------------------------------------------------------------
   !BOC
      ! Enter spatial loops (if any)
         _HORIZONTAL_LOOP_BEGIN_
          ...
          kmicrob = LOG(2.0_rk)/DT50M
          cratem = -(kmicrob/3600.0_rk)*exp(-kmicrob/3600.0_rk) !change rate per second due to microbial degradation
          ! Sedimentation
          if (self%do_sed) then
            ...
            !_SET_BOTTOM_ODE_(self%id_c_bot, +self%w*c)
                ...
                _SET_BOTTOM_ODE_(self%id_c_bot, +min(h/max_dt,self%w)*c-erosion+c_bot*cratem)
